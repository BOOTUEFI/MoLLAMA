...
def _extract_version(text: str) -> Optional[str]:
    """Pull the first x.y.z semver out of any string."""
    match = re.search(r"(\d+\.\d+\.\d+)", text)
    return match.group(1) if match else None


def _version_from_image(image) -> Optional[str]:
    """Try to read a version label from a Docker image object."""
    try:
...
def get_main_node() -> Optional[tuple[str, str]]:
    ...
    for name, inst in data.items():
        if inst.get("is_main") and inst.get("active") and inst.get("base_url"):
            if _health.get(name, False) and now >= _banned_until.get(name, 0):
                return name, inst["base_url"]
    return None


def promote_main_node() -> Optional[tuple[str, str]]:
    """
